...
`SkillsRing` (`components/SkillsRing.tsx`) orbits a set of skills items around the character in real 3D perspective — a genuine Three.js scene (camera, group, per-object rotation), reintroduced to the stack at explicit user direction after having been removed entirely earlier in the build (see Overview). The user was offered CSS 3D transforms first (`perspective` + `rotateY`/`translateZ`, no new dependency, would have kept the "no 3D scene graph" rule intact) and chose Three.js anyway — that's a real, deliberate reversal of an earlier decision, not an oversight, so don't "fix" it back to CSS without asking first.

Deliberately `CSS3DRenderer`, not the WebGL renderer: it runs the same Three.js scene/camera/perspective math, but outputs real DOM text positioned via CSS `matrix3d` transforms rather than rasterizing to a canvas — the ring uses the site's actual mono font at full crispness at any size, instead of baking text into a canvas texture. A flat marquee ticker used to sit above the nav showing the same items as the permanent, always-visible, accessible list — it was removed once the ring shipped, since duplicating the same items right next to the ring read as redundant. The same items are still listed plainly, elsewhere on the page, in the Skills Grid section; the ring itself stays `aria-hidden` and purely decorative, desktop/fine-pointer/no-reduced-motion only (see gating below).

Six items, doubled to twelve slots positioned evenly around a circle (`angle = i/count × 2π`) — direct feedback that the original single-pass-six spacing (60° apart, usually only one label in view) read as sparse individual tags rather than a continuous run of text. The whole ring spins continuously (`rotation += dt × 0.22`), and each glyph's opacity is set by hand from its current world angle (`0.35` to `1.0`, based on `cos(angle)`) since `CSS3DRenderer` doesn't shade objects by depth the way the WebGL renderer would with fog — there's no automatic depth cue to lean on here.

**One `CSS3DObject` per glyph, not per word.** The first pass gave each word its own single flat DOM card, rotated as a whole to face outward tangent to the ring — that reads fine head-on, but a word is a rigid plane, not a curve: at any angle off-center it foreshortens as one flat rectangle rather than actually following the cylinder's surface, and direct feedback ("tách lẻ từng chữ cái... để nó không bị phẳng đối với 1 cụm từ" — split out each letter so a whole phrase doesn't read flat) called this out. The fix splits each word into its own run of glyphs — `[...text, null, "◆"]`, a `null` entry consuming an angle step with no element to leave a gap before the trailing dot — and gives each glyph its own `CSS3DObject`, positioned and rotated (`object.rotation.y = angle`) at its own point on the circle, `CHAR_ANGLE_STEP` (`0.033` rad, chosen the same way the item-spacing fix below was — arc length = angle × RADIUS, tuned to roughly one monospace glyph's width at this radius) apart, centered on the word's slot angle. The word now genuinely curves along the ring's surface glyph by glyph, and — as a side effect — occlusion against the character also happens per glyph instead of per word, which is more correct than it sounds: a word straddling the front/back boundary is now partly hidden and partly visible exactly where it crosses, the way text actually wrapped around a solid cylinder would be, not an all-or-nothing swap for the whole word.

**Packing tighter needed a bigger radius, not just more items.** Naively tripling the item count at the original `RADIUS = 220` packed labels every ~20° instead of every ~60° but made adjacent items overlap and run together illegibly — an item near the front of the ring is magnified by the perspective (it's closer to the camera than the ring's own center, not sitting flat at a 1:1 CSS-pixel size the way it would at the world origin), so the apparent width at the point where legibility matters most is larger than naive arc-length math over `RADIUS × Δangle` suggests. `RADIUS = 380` with the item count only doubled (not tripled) gave enough arc length per item at that magnified front-of-ring size to stay legible while still reading denser than the original. If this needs retuning again, verify by screenshot at several rotation phases, not just the math — the front-magnification factor makes it easy to still get this wrong analytically.

**`RADIUS` also governs how wide the ring reads on screen, not just letter spacing.** Direct feedback that the ring read as too wide — spilling out toward the character's shoulders and past them — led to bringing `RADIUS` back down from `380` to `300`: a smaller radius places every glyph physically closer to the ring's own center, which reads as a tighter loop around the character rather than a wide halo around him. `CHAR_ANGLE_STEP` was scaled up in proportion (`0.026 → 0.033`, i.e. by `380/300`) in the same move, so each glyph keeps roughly the same arc-length width despite sitting on a smaller circle — narrowing `RADIUS` alone, without this, would have packed glyphs closer together in world space and started overlapping them again the same way the original too-many-items attempt did.
...
**Positioning had to fight the character's own silhouette even with real occlusion available.** The ring's glyphs all sit at local `y = 0` within their group, so before any tilt they'd trace one flat line across the screen — centering that line low, on the character's full torso, is what actually lets the "loop around him" read (front glyphs sit on his jacket, back glyphs vanish into it), rather than the narrower head/shoulder band the single-scene version needed to find gaps in his silhouette. `bottom: 0; height: 86%` (matching `.hero-character`'s own box) once occlusion made the character's width a feature instead of an obstacle to dodge.

**`TILT_ANGLE` inclines the whole ring, Saturn-ring style.** Both groups' `rotation.x` is set to a constant `TILT_ANGLE` (`14°`) alongside the per-frame `rotation.y` spin, so the ring's plane is inclined rather than perfectly horizontal — glyphs rise and fall as they orbit instead of only sliding sideways along one flat line, which reads as a genuine tilted disc around the character rather than a marquee wrapped into a cylinder. This is the knob to turn for "the ring's angle": increase it for a steeper incline, decrease toward `0` to flatten it back to a flat horizontal circle. Tilting about the local origin drags the near (front, most visually prominent) side down by `RADIUS × sin(TILT_ANGLE)` — the group's `position.y` is nudged up by that same `tiltLift` amount so the front of the ring settles back near its original untilted height instead of drooping toward the character's waist; the far side ends up correspondingly higher (`2 × tiltLift` above its untilted height), which is fine since it's the less prominent, often-occluded side.
