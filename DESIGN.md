...
`HeroSmoke` renders one plain-WebGL fullscreen-triangle fragment shader (`components/HeroSmoke.tsx`). First cut used isotropic fbm noise (soft wisps drifting in place); the user compared it against the reference directly and called it out as too diffuse and multi-directional, so it was rewritten around one fixed diagonal: the shader works in a frame rotated -13° from screen space, and renders parallel lane-shaped ribbons along that axis (`fract()` of a scaled across-flow coordinate, sharpened with `pow()`) rather than blobby noise — each lane wobbles slightly along its length and fades in and out via a second fbm field, so it reads as flowing light catching a surface, not a static striped pattern or an amorphous cloud. Density is pushed hard toward opaque at each ribbon's core, which blends toward white (`mix(tint, vec3(1.0), pow(density, 3.0) * 0.6)`) for the reference's near-white hot centers, teal↔gold at the edges. A region falloff concentrates the effect through the hero's lower-right (`fuv.y - fuv.x * 0.25` against a wide `smoothstep`) and an edge falloff keeps it clear of the copy column in the left third (`smoothstep(0.24, 0.56, vUv.x)` — pushed right of where the raw region falloff alone would put it, specifically so the ribbons don't wash out the pitch text). Rendered at `min(devicePixelRatio, 1.5)` into a canvas sized to its own CSS box (`.hero-smoke-canvas`, same bottom offset as `.hero-streaks`, taller to give the diagonal ribbons room) and composited with `mix-blend-mode: screen`. Deliberately raw WebGL, not Three.js: a single shader pass over one triangle needs none of Three.js's scene graph, camera, or material system, so the dependency isn't worth reintroducing for it. Desktop/fine-pointer/no-reduced-motion only, feature-detected before the context is ever touched (matching the pattern the old `HeroScope` used for its shader), paused via `visibilitychange` when the tab is hidden, and fully torn down (program, shaders, buffer, `ResizeObserver`) on unmount. `.hero-streaks` is not a loading state for this — it's the permanent, always-rendered base layer everywhere `HeroSmoke` can't or won't run (touch, reduced-motion, WebGL failure, mobile's `display: none` on the canvas), so the bottom band is never empty.
