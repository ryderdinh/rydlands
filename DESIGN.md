...
`HeroSmoke` renders one plain-WebGL fullscreen-triangle fragment shader (`components/HeroSmoke.tsx`). Went through three passes against direct user feedback comparing it to the reference each time: isotropic fbm noise (too diffuse, no consistent direction) → noise-derived "lanes" via `fract()` of a scaled coordinate (still read as thin sparse lines, not thick ribbons, no matter how the thresholds were tuned) → the current approach, five **explicit** gaussian-band ribbons summed in a loop, each with its own bent centerline (`fbm`-driven bend plus a `sin` wobble, not a rigid straight line) drifting sideways over time in a frame rotated -16° from screen space (one fixed diagonal, matching the reference). Each ribbon's cross-section is `exp(-dist²/2·width²)` (soft gaussian, not a hard edge) with a narrower hot core blended toward white — kept restrained (`core * 0.45`, not a wide wash) because the character's own jacket is already near-white and a bright white band disappears into it. Ribbons composite with proper alpha-over (`1-(1-a)(1-b)`, not just summed) so overlaps read as brighter, not blown out. The canvas box itself is short (42% of the hero, `.hero-smoke-canvas`) and the ribbons' base offsets are biased low, because the reference's trails sit low in the frame — early passes let the effect sweep up across the character's chest, where it fought with his jacket instead of reading as its own layer. An edge falloff (`smoothstep(0.2, 0.5, vUv.x)`) keeps it clear of the copy column in the left third. This remains an approximation of a hand-painted splash-art effect, not a pixel match to it — a procedural shader has real limits here that more parameter tuning won't close alone.

**Known limitation.** The offset math has already had one real bug (two of five ribbons' centerlines computed entirely outside the visible frame, so only one was ever seen regardless of width/color tuning — fixed by recomputing the spread against the frame's actual visible range). Future adjustments to `baseOffset`, `width`, or the rotation angle should sanity-check that every ribbon's centerline actually falls within `fuv.y`'s visible range before assuming a color or width change didn't land.
