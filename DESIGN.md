...
- Three animation libraries, three disjoint jobs — no overlap, no redundancy, and no scene-graph rendering library: Three.js stays out, though one hero effect (`HeroSmoke`) uses plain WebGL directly for a single fullscreen-shader pass, which doesn't need a scene graph to justify the dependency.
...
The hero's centerpiece is a commissioned illustrated bust portrait (`public/ryder-portrait.png`/`.webp`, transparent background, pre-trimmed to its content bounds) standing right-of-center. It's framed by four gold corner brackets (`.hero-frame-corner`, 2px `--gold` L-strokes) at the hero's true corners, mirrored ghosted "RYDER" wordmarks running both edges (`.hero-edge-lockup--left`/`--right`, outline-only Oswald, wide desktop only — narrower desktop widths lose the `.container` margin the left one needs to clear the masthead/pitch text underneath; the right one is deliberately off-center, below `.scroll-hud`'s vertical-center track rather than fighting it for the same space), and a low band of flowing teal/gold color across the frame's bottom: always-on CSS light streaks (`.hero-streaks`) with an animated noise-flow shader layered on top where it can run (`HeroSmoke` — see below). Behind all of it, `.hero-ghost-wall` tiles six rows of outline-only "RYDER" at a slight rotation — the backdrop texture, replacing an earlier live Three.js shader material with the same repeated-wordmark technique the Valorant reference actually uses (see Overview). Together these read as a game "agent reveal" card rather than a generic hero photo — a deliberate, user-directed escalation past the system's usual restraint (see Colors — Named Rules for what that costs and where the line still holds). The character is hidden entirely below 900px width: its absolute positioning sizes itself against `.hero-pin`'s 100vh box, which mobile's plain stacked flow doesn't have, so naive scaling buried the copy under a full-resolution portrait — a right-sized non-overlapping mobile treatment is future work, not yet built. The ghost wall and frame chrome still show on mobile at a smaller scale; `HeroSmoke` doesn't (see below), but `.hero-streaks` alone still covers that band there.

**Stacking note.** The hero's paint order, back to front: `.hero-ghost-wall` (auto, no z-index) → `.hero-scrim` (1) → `.hero-character` (1, after scrim in DOM) → `.hero-edge-lockup`/`.hero-streaks`/`.hero-smoke-canvas` (2) → `.hero-frame-corner` (3) → `.hero-vignette` (4, so the scene's cut to black covers the character and every frame device, not just the backdrop). A new floating hero element should slot into this scale rather than pick an unrelated number. (History: this section used to warn about a stacking-context trap from the Three.js canvas's container forcing an explicit `z-index: 0` — that container is gone along with the shader, so the trap can't recur here, but the same rule applies to any future `position: absolute` + explicit `z-index` wrapper: it isolates its children's stacking from everything outside it, regardless of their own z-index.)

### Hero Smoke (signature component)
`HeroSmoke` renders one plain-WebGL fullscreen-triangle fragment shader (`components/HeroSmoke.tsx`): a "comet tail" of 6 strands flaring from one source point low-right in the frame, not a band of parallel stripes. Went through several passes against direct user feedback comparing it to the reference each time: isotropic fbm noise (too diffuse, no direction) → noise-derived "lanes" via `fract()` (thin and sparse regardless of threshold tuning) → additive gaussian-band ribbons summed with alpha-over (`1-(1-a)(1-b)`) — this one looked right in isolation but washed into one indistinct glow with no readable strand structure once several bands overlapped on the actual page, which is exactly what made the flow direction unreadable → the current approach. Each strand computes `along`/`across` distance from the source along a fixed flow direction (`flowAngle = radians(148.0)`, steep and up-left), bends its centerline over distance and time via `fbm` (not a rigid line), and gets a **sharp-edged** cross-section — `exp(-(|d|/width)^2.2)` core plus a wider, dimmer skirt — rather than a plain gaussian, which always read as a blurred smudge no matter how narrow it was made. Width and brightness both taper with distance from the source (`reach = 1.35`), so each strand is thick and saturated near it and thins to a pale wisp by the tail's end. Strands combine with **`max()`**, not accumulation: picking the brightest strand per pixel keeps individual strands visually distinct wherever they cross, instead of stacking into a blob. A hard ceiling (`maxAlpha = min(maxAlpha, 0.92)`) caps how bright any single pixel gets, since `mix-blend-mode: screen` compounds much faster over the character's near-white jacket than it does over the dark background a shader gets tuned against in isolation — a fix worth remembering if this is retuned again. The canvas box is tall (82% of the hero, `.hero-smoke-canvas`) so the tail has room to sweep visibly across the frame. An edge falloff (`smoothstep(0.14, 0.42, vUv.x)`) keeps it clear of the copy column in the left third. This remains an approximation of a hand-painted splash-art effect, not a pixel match to it — a procedural shader has real limits here that more parameter tuning won't close alone.

**Debugging note.** Because `mix-blend-mode: screen` over the character's jacket behaves very differently from the same shader viewed over a flat dark background, tuning this by eye against an isolated render (a standalone WebGL harness with no page compositing) is necessary but not sufficient — always confirm the final look on the actual composited page, not just in isolation, before treating a shape/width change as done.

Rendered at `min(devicePixelRatio, 1.5)` and composited with `mix-blend-mode: screen`. Deliberately raw WebGL, not Three.js: a single shader pass over one triangle needs none of Three.js's scene graph, camera, or material system, so the dependency isn't worth reintroducing for it. Desktop/fine-pointer/no-reduced-motion only, feature-detected before the context is ever touched (matching the pattern the old `HeroScope` used for its shader), paused via `visibilitychange` when the tab is hidden, and fully torn down (program, shaders, buffer, `ResizeObserver`) on unmount. `.hero-streaks` is not a loading state for this — it's the permanent, always-rendered base layer everywhere `HeroSmoke` can't or won't run (touch, reduced-motion, WebGL failure, mobile's `display: none` on the canvas), so the bottom band is never empty.
