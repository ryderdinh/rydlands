...
`HeroSmoke` renders one plain-WebGL fullscreen-triangle fragment shader (`components/HeroSmoke.tsx`) — fractal Brownian motion noise, domain-warped for a flowing rather than static look, animated by `uTime`, mixed between `--teal` and `--gold`, and shaped by two `smoothstep` falloffs (vertical: strongest at the band's bottom edge, fading to nothing by its top; horizontal: soft at both side edges) so it reads as wisps of color drifting up from the frame's bottom rather than a static gradient. Rendered at `min(devicePixelRatio, 1.5)` into a canvas sized to its own CSS box (`.hero-smoke-canvas`, same footprint as `.hero-streaks`) and composited with `mix-blend-mode: screen`. Deliberately raw WebGL, not Three.js: a single shader pass over one triangle needs none of Three.js's scene graph, camera, or material system, so the dependency isn't worth reintroducing for it. Desktop/fine-pointer/no-reduced-motion only, feature-detected before the context is ever touched (matching the pattern the old `HeroScope` used for its shader), paused via `visibilitychange` when the tab is hidden, and fully torn down (program, shaders, buffer, `ResizeObserver`) on unmount. `.hero-streaks` is not a loading state for this — it's the permanent, always-rendered base layer everywhere `HeroSmoke` can't or won't run (touch, reduced-motion, WebGL failure, mobile's `display: none` on the canvas), so the bottom band is never empty.
