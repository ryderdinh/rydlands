...
Single-page Next.js site (`app/page.tsx`) with sections: nav, skills marquee, hero (pinned "agent reveal" card — illustrated character portrait, gold corner-bracket frame, ghosted repeated-wordmark backdrop), project case studies (4, pinned horizontal filmstrip), skills grid, about, contact, footer. Built with Next.js and GSAP (custom cursor, magnetic buttons, tilt cards, pinned scroll-scrubbed hero and filmstrip) plus Motion (nav active-link layout animation) and anime.js (skills-grid entrance) for interaction/motion. Three.js was in the original build (a live procedural shader material in the hero) and was later removed entirely at the user's direction in favor of the illustrated-character hero. Copy is in English.

## Capabilities and Constraints

- Stack is fixed: Next.js (App Router), React, TypeScript, GSAP, Motion, anime.js — do not introduce a different animation/rendering framework without discussing it first. Three.js was removed from the stack (see Operating Context); don't reintroduce it or another 3D/canvas library without discussing it first.
